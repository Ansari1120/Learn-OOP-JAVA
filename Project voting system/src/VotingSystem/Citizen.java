...
public class Citizen { // Super class
	...
	class Asking extends Citizen{//sub class of citizen class
		...
		void method1_inputs() {
			System.out.println("how many people to vote are there enter their numbers : ");
		    n=scn.nextInt();
			for(int i=0;i<=n;i++) {
			...
			if(isNumeric(stn)) {
	            ...
	            System.out.println("Enter your CNIC Number : ");
			        ...
			        if (strNums != null) {
			            for (int i2 = 0; i2 < stn2; i2++) {
			                try {
			                    num[i2] = Integer.parseInt(strNums[i]);//checking integers
	...
	class VoteResults extends Citizen{
		public int[] cand_num;
		int num2;
		VoteResults(String stn,int num2,int cand_num3) {
			super(stn);
			// TODO Auto-generated constructor stub
			super.cand_num=cand_num3;
			cand_num=new int[num2];
		}
		public void cast_vote(int num) {
			
			if(num2 < 0 || num2>=cand_num.length)
				return;
			cand_num[num2]=cand_num[num2]+1;
		}
		public int get_total_votes_for(int num2) {
			if(num2<0||num2>=cand_num.length)
			return 0;
			
			return cand_num[num2];
		}
		public int get_winner() {
			int largestsofar=cand_num[0];
			int winner=0;
			for(int loc=1;loc<cand_num.length;loc++) {
				if(cand_num[loc]>largestsofar) {
					winner=loc;
					largestsofar=cand_num[loc];
				}
			}
			return winner;
		}
...
VoteResults r=c.new VoteResults(stn, 10, cand_num);
r.cast_vote(cand_num);
System.out.println("candidate got : "+r.get_total_votes_for(0));
System.out.println("the winner is : "+r.get_winner());
    }
}